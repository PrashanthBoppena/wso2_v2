...
public class RetryFlowMediator extends AbstractMediator  {
	...
	public boolean mediate(MessageContext messageContext) {
		...
		try
		{
			...
			//System.out.println("==>"+requestLogApiInfo.size()+":================requestLogApiInfo=============>"+requestLogApiInfo);
			ApiLogHandlerDto requestApiInfo=requestLogApiInfo.get(0);
			String apiEndPoint=requestApiInfo.getApi_name();
			//System.out.println("api endpoint name is------------>::"+apiEndPoint);
			...
			if(apiMasterList!=null)
			{
				for (int i = 0; i < apiMasterList.size(); i++) {
					...
					ApiLogHandlerDto apiResponseObj = null;
					Optional<ApiLogHandlerDto> optionalApiResponse = Optional.empty();

					for (ApiLogHandlerDto entity : requestLogApiInfo) {
					    if (entity.getProcessId().equalsIgnoreCase(String.valueOf(apidto.getProcess_id())) &&
					        entity.getProcessName().equalsIgnoreCase(apidto.getProcess_name()) &&
					        entity.getApi_name().equals(apidto.getSub_endpoint())) {
					        
					        optionalApiResponse = Optional.of(entity);
					        break; // Exit the loop once a matching entity is found
					    }
					}

					apiResponseObj = optionalApiResponse.orElse(null);
					...
					if(apiResponseObj==null )						
					{
						...
						clientResponse = makePostRequest(sourceRequestID,DBinfoConstants.COMPOSITE_HOST_ADDRESS+compositeEndpoint, apiInfoObj,requestApiInfo.getRequest(),aggregateResponse.toString());
					...
					else if( (apiResponseObj.getStatus().equals("500")  && !(apiResponseObj.getSource_req_id().equalsIgnoreCase(apiResponseObj.getChild_req_id())) ))
					{
						...
						apidto.setSource_req_id(apiResponseObj.getSource_req_id());
						apidto.setChild_req_id(apiResponseObj.getChild_req_id());
						apidto.setLogStatus(2); // 0-disabled,1-enabled,2-response-update
						String apiInfoObj = mapper.writeValueAsString(apidto);

					//	//System.out.println("===================apiInfoObj=========>"+apiInfoObj);
					...
					}else if(!(apiResponseObj.getSource_req_id().equalsIgnoreCase(apiResponseObj.getChild_req_id())))
					{	
						//System.out.println("apiResponseObj is success::200");
						if(apiResponseObj.getStatus().startsWith("20"))
						{
							if(apiResponseObj.getResponse().startsWith("<"))
							{
							aggregateResponse.put(apiResponseObj.getProcessName(), apiResponseObj.getResponse());	
							}else
							{
								aggregateResponse.put(apiResponseObj.getProcessName(),new JSONObject( apiResponseObj.getResponse()));
							}
						}
	...
	private Map<String,String> makePostRequest(String sourceRequestID,String endpoint, String apiConfig,String inputNBPayload,String params) throws Exception {
		...
		log.info("=======request object endpoint::"+endpoint);
	...
	public static String convert(Map<String,String> json) throws JSONException {
		...
		{
			//System.out.println("convert object is::=============>"+set.getKey()+":"+set.getValue());

			if(set.getValue().startsWith("<"))
			{

				res  = res+"<"+set.getKey()+">"+ set.getValue()+"</"+set.getKey()+">"; 
			}
